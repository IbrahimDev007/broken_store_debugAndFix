...
const showProducts = (products) => {
   ...
   for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h2>Price: $ <span id=${product.id}>${product.price}</span></h2>
...
const showProductDetailsInModal = (product_details) => {
   ...
   setInnerTextDesc ('exampleModalLabel', product_details.title);
   setInnerText('product_Id', product_details?.id);
   setInnerTextDesc('modal_body', product_details?.description);
   setInnerTextDesc('rating', product_details?.rating?.rate);
};
...
const setInnerTextDesc = (id, value) => {
